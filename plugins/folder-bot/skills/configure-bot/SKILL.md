...
**무로그 불발 폴백** — 재기동 후 MCP 로그 파일이 **아예 안 생기는** 증상은 Claude Code
본체 문제다(2026-08-06 원인 격리 실측): 채널이 연결돼 있던 세션을 `tmux kill-session`
류로 강제 종료하면 그 봇 이름에 **유령 리스**가 남아, 같은 이름의 새 세션이 채널 MCP
연결을 조용히 건너뛴다(로그·재시도·오류 표시 전무). 리스는 실측 약 90분 뒤 만료된다.

1. 재기동은 가급적 `bot-restart <세션명>`(pane 교체)으로 한다 — kill-session 기반
   `stop`→`start`와 달리 유령을 만들지 않는 것이 실측됐다(하네스·folder-bot 공통).
2. 무로그 불발이 이미 발생했다면 **같은 이름 재기동 반복·예열은 효과 없다**
   (과거 "예열 복구" 성공은 리스 만료 시점과 겹친 우연으로 재해석). 약 90분 뒤
   재기동하거나, 급하면 임시로 다른 세션명·이름으로 기동해 리스 문제인지 확인한다.
3. 로그가 생기는데 `Connection failed`면 통상 원인이다 — 토큰 오입력 /
   MESSAGE CONTENT INTENT 미설정 / 서버 초대 누락을 `doctor`와 함께 점검.
   수동 연결 확인은 반드시 아래 형태로(`DISCORD_STATE_DIR` 없이 실행하면 서버가
   전역 `.env`를 찾다 죽고 그 실패 로그가 doctor 판정을 오염시킨다):
...
- **이 폴더에서 `claude`를 띄워 등록했다면 등록이 끝난 뒤 그 세션을 `/exit`한다** — 같은 폴더의
  토큰 없는 세션이 discord 플러그인을 물고 있으면 "인증 필요"가 전역 캐시돼 봇 세션 연결이 막힐 수 있다
  (bot-up이 기동 직전 캐시를 걷지만, 세션이 살아 있으면 다시 오염된다. 2026-09-11 컨테이너 실측).

### 8. 스레드 = 독립 세션 (0.1.9, claude 엔진)
채널 아래 디스코드 스레드는 스레드마다 전용 Claude 세션이 봇 tmux 세션의 창 `t<스레드ID 끝 6자리>`에
뜬다. 설치는 add가 다 한다(`bot-thread`·훅 2종·지침 블록) — 사용자에게는 이렇게 안내:
- 스레드에 메시지를 쓰면 "이 스레드는 전용 세션 …이 담당" 안내 뒤 그 세션이 답한다. 메인 채널은 그대로 메인 세션.
- 터미널에서 보려면 `tmux attach -t <세션>` 후 창 이동. 스레드 세션 창은 유휴 6시간이면 닫히고 다음 메시지에
  `--resume`으로 복원된다(맥락 유지).
